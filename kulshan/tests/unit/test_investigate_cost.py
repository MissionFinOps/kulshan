...
def _patch_success(monkeypatch, estimate_bytes: int = 70_000) -> None:
    monkeypatch.setattr(
        ...
        lambda s3_uri, billing_period, session=None: _manifest(estimate_bytes),
        raising=False,
    )
...
def test_analyze_cost_s3_reads_manifest(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "kulshan.cur.manifest_reader.read_manifest_uri",
        lambda s3_uri, billing_period, session=None: calls.append((s3_uri, billing_period)) or _manifest(),
        raising=False,
    )
...
def test_analyze_cost_scan_over_threshold_requires_confirm(monkeypatch) -> None:
    monkeypatch.setattr(
        "kulshan.cur.manifest_reader.read_manifest_uri",
        lambda s3_uri, billing_period, session=None: _manifest(200),
        raising=False,
    )
...
def test_analyze_cost_scan_under_threshold_runs(monkeypatch) -> None:
    monkeypatch.setattr(
        "kulshan.cur.manifest_reader.read_manifest_uri",
        lambda s3_uri, billing_period, session=None: _manifest(),
        raising=False,
    )
    _patch_query(monkeypatch)
...
def _patch_query(monkeypatch, estimate: ScanEstimate | None = None) -> None:
    ...
    monkeypatch.setattr("kulshan.cur.s3_query.connect_s3_duckdb", lambda session=None: FakeCon(), raising=False)
    monkeypatch.setattr(

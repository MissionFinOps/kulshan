"""Session, caching, and parallel helpers for cloud API calls."""
...
CACHE_DIR = os.path.join(os.path.expanduser("~"), ".Kulshan", "tag", "cache")
CACHE_TTL = 3600
...
def get_session(profile=None, role_arn=None):
    ...
    if role_arn:
        ...
        creds = sts.assume_role(RoleArn=role_arn, RoleSessionName="Kulshan-tag-scan")["Credentials"]
...
def paginate_all(client, method, key, **kwargs):
    """Paginate through all results for an API call."""
    results = []

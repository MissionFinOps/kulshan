...
def get_session(profile=None, role_arn=None):
    ...
    if role_arn:
        ...
        creds = sts.assume_role(RoleArn=role_arn, RoleSessionName="Kulshan-topo-scan")["Credentials"]
...
def get_enabled_regions(session):
    ...
    except Exception: return ["us-east-1", "us-west-2", "eu-west-1"]
...
def safe_api_call(client, method, **kwargs):
    ...
    for attempt in range(retries):
        try:
            ...
            if isinstance(result, dict): result.pop("ResponseMetadata", None)
            return result, None
        except botocore.exceptions.ClientError as e:
            code = e.response["Error"]["Code"]
            if code in ("Throttling", "TooManyRequestsException", "RequestLimitExceeded"):
                time.sleep(2 ** attempt); continue
            if code in ("AccessDeniedException", "AccessDenied"): return None, f"Access denied: {method}"
            return None, f"{code}: {e.response['Error']['Message']}"
        except botocore.exceptions.EndpointConnectionError: return None, "Region not available"
...
def paginate_all(client, method, key, **kwargs):
    ...
    try:
        ...
        for page in paginator.paginate(**kwargs): results.extend(page.get(key, []))
    except Exception as e: return results, str(e)
    return results, None
